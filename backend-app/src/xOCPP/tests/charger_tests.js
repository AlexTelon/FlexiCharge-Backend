...
module.exports = function ({ ocppInterface, constants, v, func, messageValidations }) {
    ...
    exports.connectAsChargerSocket = function (connectorID, callback) {
        try {
            console.log('\n========= CHARGER MOCK CONNECTING... ==========\n')
            const ws = new WebSocket("ws://localhost:1337/charger/abc113")

            ws.on('open', function open() {
                v.addConnectedChargerSockets(connectorID, ws)
                callback(ws)
            })
    ...
    exports.checkChargerClientsMemoryLeak = function (origin, callback) {
        if (v.getLengthConnectedChargerSockets() || v.getLengthChargerSerials() || v.getLengthconnectorIDs()) {
            console.log("(MEMORY TEST FAILED) Number of connected chargers: " + v.getLengthConnectedChargerSockets() + " (" + v.getLengthChargerSerials() + ")" + " (" + v.getLengthconnectorIDs() + ")")
    ...
    exports.testRemoteStart = function (connectorID, callback) {
        currentTest = c.REMOTE_START_TRANSACTION
        testSuccessful = false
        testValidationSuccessful = true

        console.log("\n========= TESTING REMOTE START... ==========\n")
        ocppInterface.remoteStartTransaction(connectorID, 1, function (error, response) {
            if (error != null) {
                console.log("\nError: " + error)
            } else {
    ...
    exports.testRemoteStop = function (connectorID, callback) {
        currentTest = c.REMOTE_STOP_TRANSACTION
        testSuccessful = false
        testValidationSuccessful = true

        console.log("\n========= TESTING REMOTE STOP... ==========\n")
        ocppInterface.remoteStopTransaction(connectorID, 1, function (error, response) {
            if (error != null) {
                console.log("\nError: " + error)
            } else {
    ...
    exports.testReserveNow = function (connectorID, callback) {
        currentTest = c.RESERVE_NOW
        testSuccessful = false
        testValidationSuccessful = true

        console.log("\n========= TESTING RESERVE NOW... ==========\n")
        ocppInterface.reserveNow(connectorID, c.CONNECTOR_ID, c.ID_TAG, c.RESERVATION_ID, c.PARENT_ID_TAG, function (error, response) {
            if (error != null) {
